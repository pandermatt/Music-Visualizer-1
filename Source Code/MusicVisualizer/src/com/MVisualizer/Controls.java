...
public class Controls {
    ...
    public static void setupControls(){
        ...
        createButton(g1, (soundCloudApi != null) ? "soundcloud" : "connect soundcloud", 4, 0, h * 10 + spacing, 85, 15, new IControlEvent(e -> {
            if (soundCloudApi == null) {
                Button this_ = myButtons[4];
                try {visualizerRef.initSoundCloud();}
                catch (Exception e1) {
                    EException.append(e1);
                Controls.visualizerRef.showError("Could Not Connect To SoundCloud at this time :( Check Connection and Try Again",
                        "SoundCloud Connection Error");}

                if (soundCloudApi == null) this_.setCaptionLabel("connect soundcloud");
                else {this_.setCaptionLabel("soundcloud"); URLLoader.getInstance();}
            }
